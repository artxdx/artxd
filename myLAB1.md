...
R1(config)# enable password scrypt cisco12345 - устанавливаем пароль для привелегированного режима

## _Вопрос:
## Как настройка секретного пароля включения помогает защитить маршрутизатор от взлома в результате атаки?_
Пароль должен быть сложным(для подбора),шифрованным.Пароль необходим для защиты настроек маршрутизатора.
...
username user01 secret ?  
username user01 password ?  
## _Вопрос: Какие варианты доступны?_  
R1(config)# username user01 secret ?  
0     Specifies an UNENCRYPTED secret will follow  - незашифрованный пароль  
5     Specifies a HIDDEN secret will follow  - зашифрованный пароль (MD5 - 5 уровень шифрования)  
LINE  The UNENCRYPTED (cleartext) user secret   - текст пароля    

R1(config)# username user01 password ?  
0     Specifies an UNENCRYPTED password will follow - незашифрованный пароль    
7     Specifies a HIDDEN password will follow  - зашифрованный пароль (MD7 - 7 уровень шифрования)
LINE  The UNENCRYPTED (cleartext) user password  -  текст пароля
...
enable password 7 08324F5C10091157465958567E79252E3B3D
## _Вопрос:
## Какой метод хеширования используется для пароля?_
md5
...
## Введите свои ответы здесь._
Запрашивается логин и пароль.Раньше запрашивался только пароль.
### d. После входа в систему выполните команду show run.
## _Вопрос:
## Смогли ли вы отдать команду? Объяснить.
## Введите свои ответы здесь._
Да.
